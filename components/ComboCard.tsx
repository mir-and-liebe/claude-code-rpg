...
export function ComboCard({ combos, totalCombos = 6 }: Props) {
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-[11px] text-text-muted ml-auto font-mono">
          {combos.length}/{totalCombos} discovered
        </span>
      </div>
      {combos.length === 0 ? (
        <p className="text-[12px] text-text-muted py-2">
          Complete skills from different trees to discover cross-tree combos and earn bonus XP.
        </p>
      ) : (
